...
function slotsForDay(dateStr, ctx, now) {
  ...
  // Departures stay on the calendar date even when a window runs past midnight.
  const nextMidnightUtc = ptToUtc(addDays(dateStr, 1), '00:00');

  const result = { options: {}, callToBook: false };
  // A seasonal tour (lobster diving) has no slots at all outside its season.
  if (!inSeason(dateStr, tour.season_start, tour.season_end)) return result;
  ...
  for (const rule of rules) {
    ...
    // '16:00'–'02:00' is a night trip back at 2 AM the next day.
    const windowEndUtc = ptToUtc(windowEnd <= windowStart ? addDays(dateStr, 1) : dateStr, windowEnd);

    for (const opt of options) {
      // A rule pinned to one pricing option (the night rule) skips the others.
      if (rule.pricing_option_id && rule.pricing_option_id !== opt.id) continue;
      ...
      if (startTimes && startTimes.length > 0) {
        ...
        for (
          ...
          t + durationMs <= windowEndUtc.getTime() && t < nextMidnightUtc.getTime();
          t += step
        ) {
